...
#End Region
    ...
    Public Function PrintLabel_P23(ByVal dsRequest As DataSet) As String
        ...
        Try
            ...
            LogMsg("PrintLabel_P23 part:", labelreq.lmesPartID)
            LogMsg("PrintLabel_P23 lmesMachID:", labelreq.lmesMachID)
            LogMsg("PrintLabel_P23 ReportGroupID:", labelreq.ReportGroupID)
            ...
            If (dsICS_data.Tables(0).Rows.Count > 0) Then
                If Not Destination.UseGeneric Then
                    ...
                    Try
                        ...
                        If labelreq.ScheduleRequired And Destination.QtyPkgsperSkidReq > 1 And (Destination.Package_Code = "Z" Or Destination.Package_Code = "C" Or Destination.Package_Code = "UC" Or Destination.Package_Code = "BU") And (Not Destination.UseGeneric) Then
                            'save to buffer
                            LogMsg("ready to insert mesdba.ics_label_buffer!")
                ...
                LogMsg("return FormatOutput labelfile:", labelfile)
                ...
                If labelreq.ScheduleRequired And Destination.QtyPkgsperSkidReq > 1 And Destination.Completed And ConfigurationSettings.AppSettings.Get("UseSkidSummary") = "True" _
                And (Destination.Package_Code = "Z" Or Destination.Package_Code = "C" Or Destination.Package_Code = "UC" Or Destination.Package_Code = "BU") And (Not Destination.UseGeneric) Then
                    'Generate the skid summary and append to labelfile
                    Dim skidsum As String
                    el = 318
                    LogMsg("skid summary and append to labelfile")
...
GetScheduleWP:
    ...
    Public Function FormatOutput(ByVal dsICS As DataSet, ByVal Reprint As Boolean) As String
        ...
        'for testing new GM label  *** Jerry
        dt.Columns.Add("SHIP_DATE1", GetType(System.String))
        dt.Columns.Add("DATE1", GetType(System.String))
        ' end new GM label
        Try
            ...
            For i = 0 To labelsRequiredQty - 1
                ...
                With dsICS.Tables(0)
                    ...
                    'dr.Item("SHIP_DATE") = Now.ToShortDateString  *** Jerry
                    Dim datefield As String
                    datefield = Now.Day.ToString() & Now.ToString("MMM").ToUpper()
                    dr.Item("SHIP_DATE") = datefield.ToString()
                    dr.Item("SHIP_DATE1") = Now.Year.ToString()
                    dr.Item("DATE1") = Now.ToString("yyyyMMdd")
                ...
                LogMsg("FORMATOUTPUT SERNBR: ship_package", dr.Item("SERNBR"), ship_package)
                LogMsg("FORMATOUTPUT gross_weight: quantity", gross_weight, quantity)
                ...
                If strx.CompareTo("I") = 0 Then
                    ...
                    ' Add for New T1 UEC, BDU
                    If strx.CompareTo("BU") = 0 Or strx.CompareTo("UC") = 0 Then
                        If Reprint = False Then
                            template(0) = ConfigurationSettings.AppSettings.Get(Common.GetServerType & "ApplicationPath") & ConfigurationSettings.AppSettings.Get("ICS1724A_300DPI_2D")
                        Else
                            template(0) = ConfigurationSettings.AppSettings.Get(Common.GetServerType & "ApplicationPath") & ConfigurationSettings.AppSettings.Get("ICS1724A_300DPI_2D_Reprint")
            ...
            Return ostring.ToString
        Catch ex As Exception
            LogMsg("format output err", ex.ToString())
            If ConfigurationSettings.AppSettings.Get("LogEvents") = "True" Then
    ...
    Private Sub GetWeightQuantity_For_Label(ByVal serial_nbr As String, ByVal ship_package As String, ByRef weight As Integer, ByRef quantity As Integer)
        ...
        Else
            Delphi_part_nbr = ds.Tables(0).Rows(0).Item("Delphi_part_nbr")
            shipto_nbr = ds.Tables(0).Rows(0).Item("shipto_nbr")
            LogMsg("Delphi_part_nbr:", Delphi_part_nbr)
            LogMsg("shipto_nbr:", shipto_nbr)
            LogMsg("ship_package:", ship_package)
