...
Partial Class SerialNumber_Search
...
#Else
                Case "Start Date and End Date (BEC)"
                    selDate.Append("SELECT ('Serial_History.aspx?job=BEC&StandardPack_Id=' || sp.STANDARD_PACK_ID || '') AS Serial_Content, sp.SERIAL_NBR, sp.INSERT_TMSTM, sp.REV_PHYSICAL, m.MACHINE_NAME, mp.PART_NBR, sp.LABEL_CODE, sp.STORE, sp.DISPOSITION_CODE, Regexp_Replace(ssp.SERIAL_NBR,'_|:','') as Container_SerialNbr")
                    frmDate.Append(" FROM MESDBA.STANDARD_PACK sp, MESDBA.MACHINE m, MESDBA.MES_PART mp, MESDBA.SERIALIZED_PRODUCT ssp")
                    whrDate.Append(" WHERE sp.MACHINE_ID = m.MACHINE_ID AND sp.MES_PART_ID = mp.MES_PART_ID AND sp.INSERT_TMSTM >= to_date('" & txtStart.Text & " 12:00:00 AM', 'MM/DD/YYYY HH:MI:SS AM') AND sp.INSERT_TMSTM <= to_date('" & txtEnd.Text & " 12:00:07 AM', 'MM/DD/YYYY HH:MI:SS AM') AND ssp.STANDARD_PACK_ID = sp.STANDARD_PACK_ID ")
                    If cmbDepartment.SelectedValue <> "0" Then
                        whrDate.Append("AND sp.REPORT_GROUP_ID = '" & cmbDepartment.SelectedValue & "'")
                    End If
                    If cmbMachineName.SelectedValue <> "0" Then
...
#End If
                ...
                Case "Start Date and End Date (BAG)"
                    ...
                    frmDate.Append(" FROM MESDBA.STANDARD_PACK sp, MESDBA.MACHINE m, MESDBA.MES_PART mp")
                    whrDate.Append(" WHERE sp.MACHINE_ID = m.MACHINE_ID AND sp.MES_PART_ID = mp.MES_PART_ID AND sp.INSERT_TMSTM >= to_date('" & txtStart.Text & " 12:00:00 AM', 'MM/DD/YYYY HH:MI:SS AM') AND sp.INSERT_TMSTM <= to_date('" & txtEnd.Text & " 12:00:07 AM', 'MM/DD/YYYY HH:MI:SS AM') AND ssp.PRODUCT_ID >= '" & txtStart.Text & "' AND ssp.PRODUCT_ID <= '" & txtEnd.Text & "'")
